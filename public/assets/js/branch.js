$(function() {
    ...
    function getServiceEnglish() {
        ...
        $.get("/admin/transadd_branch/english/get", function(e) {
            e.forEach(function(item) {
                $("#english-table").append(
                    `<tr data-key=${item.id} class="english-data">
                    <td>${item.transparent_id}</td>
                    <td>${item.name}</td>
                    <td><a href="/admin/transadd_branch/english/delete/${
                        item.id
    ...
    $("#english-table").on("click", ".english-data", function() {
        ...
        $.get("/admin/compane_add/english/" + id, function(e) {
            $("#english #name").val(e.compane);
            $(".card-overlay").hide();
            $("#service-submit").attr("disabled", false);
        });
    });
